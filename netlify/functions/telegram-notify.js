// Netlify Function: telegram-notify
// Sends a Telegram message WITHOUT exposing your bot token on the client.
// Set these environment variables in Netlify Dashboard → Site settings → Build & deploy → Environment:
//   TELEGRAM_BOT_TOKEN  (required)
//   TELEGRAM_CHAT_ID    (optional; fallback if not provided in request)
//   ALLOWED_ORIGINS     (optional; comma-separated list; if absent, CORS will be open "*")
...
exports.handler = async (event) => {
  ...
  if (!TOKEN) {
    return cors(500, { ok: false, error: "Missing TELEGRAM_BOT_TOKEN env var" });
  }
  ...
  if (!text) {
    return cors(400, { ok: false, error: "Missing 'text' field" });
  }
  if (!chatId) {
    return cors(400, { ok: false, error: "Missing 'chat_id' (and no TELEGRAM_CHAT_ID default)" });
  }

  try {
    if (file && file.dataUrl) {
  // Expect payload.file = { name, type, dataUrl, mode: 'document'|'photo' }
  try {
    const m = String(file.dataUrl).split(','); // "data:mime;base64,AAAA"
    if (m.length < 2) throw new Error("Invalid dataUrl");
    const meta = m[0]; const b64 = m[1];
    ...
    if (!data.ok) {
      return cors(502, { ok: false, error: "Telegram API error", details: data });
    }
    return cors(200, { ok: true, result: data.result });
  } catch (e) {
    return cors(400, { ok: false, error: "Invalid file payload: " + (e.message || String(e)) });
  }
}
